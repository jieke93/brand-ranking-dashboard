...
# 상품명 - 이미지 alt에서 추출!
            ...
            # 가격
            try:
                price_elem = tile.find_element(By.CSS_SELECTOR, "[class*='price'], [class*='Price']")
                price_text = price_elem.text.strip()
                # 숫자와 원 추출
                price_match = re.search(r'[\d,]+원', price_text)
                product['price'] = price_match.group(0) if price_match else price_text[:15]
            except:
                pass
            ...
            try:
                ...
                # 컬러명 추출 시도
                colors = []
                for chip in color_chips[:10]:  # 최대 10개
                    try:
                        img = chip.find_element(By.TAG_NAME, "img")
                        color_name = img.get_attribute("alt") or img.get_attribute("title") or ""
            ...
            # 평점 (판매 데이터)
            try:
                rating_elem = tile.find_element(By.CSS_SELECTOR, "[class*='rating-average'], [class*='rating']")
            ...
            # 리뷰 수
            try:
                review_elem = tile.find_element(By.CSS_SELECTOR, "[class*='count-product'], [class*='review']")
...
def main():
    log("=" * 60)
    log("유니클로 랭킹 크롤러 (WOMEN/MEN/KIDS/BABY)")
    log("=" * 60)
    log("\n수집 항목:")
    log("  • 상품명, 가격")
    log("  • 컬러 개수, 컬러명")
