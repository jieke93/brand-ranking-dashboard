#!/usr/bin/env python3
"""
탑텐(TOPTEN) 랭킹 수집 + 분석 통합 실행기
─────────────────────────────────────
  1단계: 크롤링 (topten_ranking_v3.py)
  2단계: 분석 + 히스토리 누적 (analyze_topten.py)

히스토리 누적 구조:
  - topten_history.json : 날짜별 랭킹 데이터 (자동 누적)
  - topten_history_backup.json : 백업 (매 실행시 자동 생성)
  - 이전 크롤링 엑셀 파일 : JSON 유실 시 자동 복구 소스

사용법:
  python run_topten.py           # 크롤링 + 분석 전체 실행
  python run_topten.py --analyze # 분석만 실행 (이미 크롤링한 데이터로)
  python run_topten.py --recover # 이전 엑셀에서 히스토리 강제 복구
"""
...
CRAWLER_SCRIPT = os.path.join(WORK_DIR, 'topten_ranking_v3.py')
ANALYZER_SCRIPT = os.path.join(WORK_DIR, 'analyze_topten.py')
...
def run_crawler():
    """크롤러 실행"""
    log("=" * 60)
    log("  [STEP 1] TOPTEN 주간베스트 크롤링 시작")
    log("=" * 60)
...
def run_analyzer():
    """분석기 실행"""
    log("\n" + "=" * 60)
    log("  [STEP 2] TOPTEN 랭킹 분석 + 히스토리 누적")
    log("=" * 60)
...
def main():
    ...
    if not crawler_ok:
        log("\n  [WARN] 크롤링이 실패했습니다.")
        log("  기존 데이터로 분석을 진행하시려면: python run_topten.py --analyze")
        return
